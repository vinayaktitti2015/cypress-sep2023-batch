...
Cypress.Commands.add('userLogin', () => {
  ...
  cy.session('userLogin', () => {
    ...
    cy.get('[name="username"]')
      .clear()
      .type(Cypress.env('username'), { log: true })
...
function debugTabState () {
  ...
  console.warn(
    '-----debugTabState: active_tab:',
    active_tab_index + ' ' + myTabNames[active_tab_index]
  )
...
Cypress.Commands.add('openTab', (url, opts) => {
  ...
  function finalize () {
    ...
    let promise = new Cypress.Promise(resolve => {
      ...
      // https://developer.mozilla.org/en-US/docs/Web/API/Window/open
      popupcounter++
      let popup = myTabs[indexNext]
        ? myTabs[indexNext]
        : window.top.open(url, windowName, opts.windowFeatures)
      ...
      function checkReady () {
        ...
        if (
          !popup.document.body ||
          popup.document.body.innerHTML.length === 0
        ) {
          setTimeout(() => {
            checkReady()
          }, 32) // arbitrary delay
...
function resolve_index_or_name_to_index (index_or_name) {
  let index =
    parseInt(index_or_name) >= 0 ? index_or_name : active_tab_index || 0
...
// or
// cy.get(loc).invoke('show').then(($el) => {})
